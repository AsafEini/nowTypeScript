...
var Ui = (function() {
    ...
    let editFileWindow = $('#text_file_container').clone();
    let myHistoryNavigation = new MyHistory();
    ...
    function initBrowser() {
        let rootUl = createNewUlWithId(0, fileSystem.getFileById(0)._name);
    ...
    function openBrowserDirectory(folderId) {
        ...
        let directory = fileSystem.getFileById(folderId);
    ...
    function addListenerClickToGoButton() {
        $('#go_to_address').click(function () {
            let searchPath = $('#root_address').val();
            ...
            while (flag && index < searchPath.length) {
                let file = fileSystem.getFileById(currentFolderId);
...
//}
    ...
    function drawingTheContentFileWithADiv(fileId) {
        ...
        let targetFile = fileSystem.getFileById(fileId);
    ...
    function goBack() { //HAKOL HADASH BA FUNKTZIA!!
        $('#back').click(function () {
            if (myHistoryNavigation._historyForGoingBack.length > 0) {
                let folderHistoryToGoBack = myHistoryNavigation.getLastIdInHistory();
        ...
        $('#fwr').click(function () {
            if (myHistoryNavigation._historyForGoingForward.length > 0) {
                let folderInHistoryToGoForward2 = myHistoryNavigation._historyForGoingForward.pop();
    ...
    function updateCurrentRootAddress(fileId) {
        ...
        while (fileId > -1) {
            let file = fileSystem.getFileById(fileId);
